...
**Phase 3**（read-only API 疎通 / 2026-05-17 に Phase 2 完了承認、進行中）。

- Phase 2 完了の根拠と判断: [docs/PHASE_2_OBSERVATION.md](docs/PHASE_2_OBSERVATION.md) 「進行判断: Phase 2 完了 → Phase 3 GO」
- dry-run 観察自体は Oracle Cloud VM で並走継続中 (停止しない、Phase 3〜6 の参照値として残す)
...
- Phase 3 進捗:
  ...
  - [ ] `GET /v1/account/assets` で残高取得疎通 — **人間作業待ち** (手順は [docs/PHASE_3_API_KEY_SETUP.md](docs/PHASE_3_API_KEY_SETUP.md))
- live 注文は **絶対に出さない** (Hard Rules / `ENABLE_LIVE_ORDER=False` / `_send_live_order` は NotImplementedError のまま)
