...
**Phase 2**（dry-run 観察 / 観察開始は Oracle Cloud で systemctl start した日 から最低 14 日）。

- 本命 = balanced_ndx champion (`config/strategies/btc_eth_balanced_ndx.yaml`)
- 並走 = 旧 champion (`config/app.yaml` = 5min trend>=5 ma=5/20, BTC 単独, `./data/compare_old`)
- 稼働基盤 = Oracle Cloud Always Free 上の Linux VM + systemd ([docs/ORACLE_CLOUD_SETUP.md](docs/ORACLE_CLOUD_SETUP.md))
  - `gmo-bot-safe-primary.service` (本命)
  - `gmo-bot-safe-compare-old.service` (並走)
  - `fetch-index-daily.timer` (NDX/SPX/VIX を毎日 22:00 UTC に更新)
- ローカル動作確認のみ = `bash scripts/dry_run_compare.sh`
